...
namespace Domain
{
   public static class ResultBoolExtensions
   {
      ...
      public static Result<TValue> ToResult<TValue>(this bool isOk, TValue value, string error) =>
         isOk ? Result.Ok(value) : Result.Fail<TValue>(error);

      public static Result<TValue> ToResult<TValue>(this bool isOk, Func<TValue> value, string error) =>
         isOk ? Result.Ok(value()) : Result.Fail<TValue>(error);
