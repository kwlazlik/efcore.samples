...
namespace IQToolkit
{
 ...
 = lambda.Body;
            ...
            var result
 = EvaluatorBuilder.Build(null, null, expression);
        ...
        public static object Eval(LambdaExpression function, params object[] args)
        {
            ...
            var result
 = EvaluatorBuilder.Build(null, function.Parameters, function.Body);
 ...
 = delegateType.GetMethod("Invoke");
            var host
 = new EvaluatorHost(outer, nArgs, evaluator);
 ...
 = values;
                this.start
 = (outer != null ? outer.start + (outer.values != null ? outer.values.Length : 0) : 0);
            }
 ...
 = state.outer;
                ...
                state.values[index - state.start]
 = value;
            }
        }
 ...
 = parameters.ToList();
            ...
            private int Count
            {
                get
                {
                    ...
                    {
                        this.count
 = (this.outer != null ? this.outer.Count : 0) + (this.parameters != null ? this.parameters.Count : 0);
                    }
                    return this.count;
                }
            }
 ...
 = new VariableFinder();
                ...
                {
                    LambdaExpression lambda
 = iv.Expression as LambdaExpression;
                    if (lambda != null)
                    {
 ...
 = Build(u.Operand);
                ...
                bool isSourceTypeNullable
 = IsNullable(u.Operand.Type);
                bool isTargetTypeNullable
 = IsNullable(u.Type);
                Type sourceType
 = GetNonNullType(u.Operand.Type);
                Type targetType
 = GetNonNullType(u.Type);
                ...
                switch (u.NodeType)
                {
                    ...
                    case ExpressionType.Not:
                        {
                            MethodInfo mi
 = FindBestMethod(Operators.GetOperatorMethods(u.NodeType.ToString()), new Type[] { sourceType }, targetType);
                    ...
                    case ExpressionType.ConvertChecked:
                        {
                            ...
                            {
                                MethodInfo mi
 = FindBestMethod(Operators.GetOperatorMethods(u.NodeType + "To" + targetType.Name), new Type[] { sourceType }, targetType);
                                Delegate fn
 = Delegate.CreateDelegate(typeof(Func<,>).MakeGenericType(sourceType, targetType), null, mi);
                                if (!isSourceTypeNullable && !isTargetTypeNullable)
                                {
                                    return (Evaluator)Activator.CreateInstance(
            ...
            {
                Delegate opFunc
 = Delegate.CreateDelegate(typeof(Func<,>).MakeGenericType(sourceType, targetType), null, method);
                if (u.IsLiftedToNull)
                {
                    return (Evaluator)Activator.CreateInstance(
 ...
 = Build(b.Right);
                ...
                Type sourceType
 = GetNonNullType(b.Left.Type);
                Type targetType
 = GetNonNullType(b.Type);
 ...
 = b.Conversion;
                            ...
                            {
                                conv
 = Expression.Lambda(Expression.Convert(conv.Body, b.Type), conv.Parameters.ToArray());
                            }
                            if (conv.Parameters[0].Type == b.Left.Type || conv.Parameters[0].Type == sourceType)
                            {
                                ...
                                {
                                    var p
 = Expression.Parameter(b.Left.Type, "left");
                                    conv
 = Expression.Lambda(Expression.Invoke(conv, Expression.Convert(p, sourceType)), p);
                                }
                                Delegate fnConv
 = ExpressionEvaluator.CreateDelegate(conv);
                                return (Evaluator)Activator.CreateInstance(
                            ...
                            else if (conv.Parameters[0].Type == b.Right.Type || conv.Parameters[0].Type == rightType)
                            {
                                ...
                                {
                                    var p
 = Expression.Parameter(b.Right.Type, "right");
                                    conv
 = Expression.Lambda(Expression.Invoke(conv, Expression.Convert(p, rightType)), p);
                                }
                                Delegate fnConv
 = ExpressionEvaluator.CreateDelegate(conv);
                                return (Evaluator)Activator.CreateInstance(
 ...
 = Expression.Parameter(b.Left.Type, "left");
                            var lambda
 = Expression.Lambda(Expression.Convert(p, b.Type), p);
                            Delegate fnConv
 = ExpressionEvaluator.CreateDelegate(lambda);
                            return (Evaluator)Activator.CreateInstance(
                                typeof(CoalesceREvaluator<,>).MakeGenericType(b.Left.Type, b.Right.Type), 
                                new object[] { opLeft, opRight, fnConv }
                                );
 ...
 = ExpressionEvaluator.CreateDelegate(lambda);
            ...
            {
                Delegate opFunc
 = Delegate.CreateDelegate(typeof(Func<,,>).MakeGenericType(sourceType, sourceType, targetType), null, method);
                if (b.IsLiftedToNull)
                {
                    return (Evaluator)Activator.CreateInstance(
 ...
 = this.FindParameterIndex(p);
            ...
            {
                for (var builder
 = this; builder != null; builder = builder.outer)
                {
                    if (this.parameters != null)
                    {
                        for (int i
 = 0, n = this.parameters.Count; i < n; i++)
                        {
            ...
            {
                var opInst
 = (mc.Object != null) ? Build(mc.Object) : null;
                var opArgs
 = mc.Arguments.Select(a => Build(a)).ToArray();
 ...
 = method.GetParameters();
                ...
                {
                    var types
 = new Type[] { parameters[0].ParameterType, method.ReturnType };
                    var fn
 = Delegate.CreateDelegate(typeof(Func<,>).MakeGenericType(types), null, method);
                    return (Evaluator)Activator.CreateInstance(
                ...
                {
                    var types
 = new Type[] { opInst.ReturnType, method.ReturnType };
                    var fn
 = Delegate.CreateDelegate(typeof(Func<,>).MakeGenericType(types), null, method);
                    return (Evaluator)Activator.CreateInstance(
                        typeof(FuncEvaluator<,>).MakeGenericType(types),
                ...
                {
                    var types
 = new Type[] { parameters[0].ParameterType, parameters[1].ParameterType, method.ReturnType };
                    var fn
 = Delegate.CreateDelegate(typeof(Func<,,>).MakeGenericType(types), null, method);
                    return (Evaluator)Activator.CreateInstance(
                        typeof(FuncEvaluator<,,>).MakeGenericType(types),
                ...
                {
                    var types
 = new Type[] { opInst.ReturnType, parameters[0].ParameterType, method.ReturnType };
                    var fn
 = Delegate.CreateDelegate(typeof(Func<,,>).MakeGenericType(types), null, method);
                    return (Evaluator)Activator.CreateInstance(
                        typeof(FuncEvaluator<,,>).MakeGenericType(types),
                ...
                {
                    var types
 = new Type[] { parameters[0].ParameterType, parameters[1].ParameterType, parameters[2].ParameterType, method.ReturnType };
                    var fn
 = Delegate.CreateDelegate(typeof(Func<,,,>).MakeGenericType(types), null, method);
                    return (Evaluator)Activator.CreateInstance(
                        typeof(FuncEvaluator<,,,>).MakeGenericType(types),
                ...
                {
                    var types
 = new Type[] { opInst.ReturnType, parameters[0].ParameterType, parameters[1].ParameterType, method.ReturnType };
                    var fn
 = Delegate.CreateDelegate(typeof(Func<,,,>).MakeGenericType(types), null, method);
                    return (Evaluator)Activator.CreateInstance(
                        typeof(FuncEvaluator<,,,>).MakeGenericType(types),
                ...
                {
                    var types
 = new Type[] { parameters[0].ParameterType, parameters[1].ParameterType, parameters[2].ParameterType, parameters[3].ParameterType, method.ReturnType };
                    var fn
 = Delegate.CreateDelegate(typeof(Func<,,,,>).MakeGenericType(types), null, method);
                    return (Evaluator)Activator.CreateInstance(
                        typeof(FuncEvaluator<,,,,>).MakeGenericType(types),
                ...
                {
                    var types
 = new Type[] { opInst.ReturnType, parameters[0].ParameterType, parameters[1].ParameterType, parameters[2].ParameterType, method.ReturnType };
                    var fn
 = Delegate.CreateDelegate(typeof(Func<,,,,>).MakeGenericType(types), null, method);
                    return (Evaluator)Activator.CreateInstance(
                        typeof(FuncEvaluator<,,,,>).MakeGenericType(types),
            ...
            {
                var operand
 = m.Expression != null ? Build(m.Expression) : null;
                FieldInfo field
 = m.Member as FieldInfo;
                if (field != null)
                {
                    return (Evaluator)Activator.CreateInstance(
 ...
 = m.Member as PropertyInfo;
                ...
                {
                    Type opType
 = operand != null ? operand.ReturnType : m.Member.DeclaringType;
                    return (Evaluator)Activator.CreateInstance(
 ...
 = Build(t.Expression);
            ...
            {
                var opArgs
 = n.Arguments.Select(a => Build(a)).ToArray();
                if (opArgs.Length == 0)
                {
                    return (Evaluator)Activator.CreateInstance(
            ...
            {
                Type elementType
 = n.Type.GetElementType();
                var initializers
 = n.Expressions.Select(e => Build(elementType, e)).ToArray();
                return (Evaluator)Activator.CreateInstance(
            ...
            {
                var bounds
 = n.Expressions.Select(e => Build(e)).ToArray();
                return (Evaluator)Activator.CreateInstance(
            ...
            {
                var evaluator
 = EvaluatorBuilder.Build(this, lambda.Parameters, lambda.Body); 
                return (Evaluator)Activator.CreateInstance(
            ...
            {
                var lambda
 = inv.Expression as LambdaExpression;
                if (lambda != null)
                {
                    // assume parameters from nested lambda area already in scope (see VariableFinder above)
                    Evaluator ev
 = this.Build(lambda.Body);

                    // make nested let expressions...
                    for (int i
 = lambda.Parameters.Count - 1; i >= 0; i--)
                    {
                        var parameter
 = lambda.Parameters[i];
                        int index
 = this.FindParameterIndex(parameter);
                        var evValue
 = this.Build(inv.Arguments[i]);
                        ev
 = (Evaluator)Activator.CreateInstance(
                ...
                {
                    var opFunction
 = new EvaluatorBuilder(this, null).Build(inv.Expression);
                    var opArgs
 = inv.Arguments.Select(a => Build(a)).ToArray();
                    return (Evaluator)Activator.CreateInstance(
            ...
            {
                var external
 = ExternalReferenceGatherer.Gather(this, u.Operand).ToDictionary(p => this.FindParameterIndex(p));
                return (Evaluator)Activator.CreateInstance(
            ...
            class ExternalReferenceGatherer : ExpressionVisitor
            {
                ...
                HashSet<ParameterExpression> external
 ...
 = builder;
                ...
                {
                    var visitor
 = new ExternalReferenceGatherer(builder);
            ...
            {
                var evNew
 = Build(mini.NewExpression);
                var initializers
 = mini.Bindings.Select(b => MemberBinding(mini.Type, b)).ToArray();
                return (Evaluator)Activator.CreateInstance(
            ...
            {
                var evNew
 = Build(lini.NewExpression);
                var initializers
 = lini.Initializers.Select(b => Element(lini.Type, b)).ToArray();
                return (Evaluator)Activator.CreateInstance(
 ...
 = (PropertyInfo)ma.Member;
            ...
            {
                FieldInfo fi
 = mb.Member as FieldInfo;
                if (fi != null)
                {
                    var initializers
 = mb.Bindings.Select(b => MemberBinding(fi.FieldType, b)).ToArray();
                    if (type.IsValueType)
                    {
                        return (Initializer)Activator.CreateInstance(
                ...
                PropertyInfo pi
 = mb.Member as PropertyInfo;
                if (pi != null)
                {
                    var initializers
 = mb.Bindings.Select(b => MemberBinding(pi.PropertyType, b)).ToArray();
                    if (type.IsValueType)
                    {
                        return (Initializer)Activator.CreateInstance(
 ...
 = mb.Member as FieldInfo;
                ...
                {
                    var initializers
 = mb.Initializers.Select(e => Element(fi.FieldType, e)).ToArray();
                    if (type.IsValueType)
                    {
                        return (Initializer)Activator.CreateInstance(
                            typeof(ValueTypeFieldMemberInitializer<,>).MakeGenericType(type, fi.FieldType),
 ...
 = mb.Member as PropertyInfo;
                ...
                {
                    var initializers
 = mb.Initializers.Select(e => Element(pi.PropertyType, e)).ToArray();
                    if (type.IsValueType)
                    {
                        return (Initializer)Activator.CreateInstance(
                            typeof(ValueTypePropertyMemberInitializer<,>).MakeGenericType(type, pi.PropertyType),
            ...
            {
                var evArgs
 = elem.Arguments.Select(a => Build(a)).ToArray();
                if (evArgs.Length == 1)
                {
                    var types
 = new Type[] { type, evArgs[0].ReturnType };
                    return (Initializer)Activator.CreateInstance(
                ...
                {
                    var types
 = new Type[] { type, evArgs[0].ReturnType, evArgs[01].ReturnType };
                    return (Initializer)Activator.CreateInstance(
                        typeof(ElementInitializer<,,>).MakeGenericType(types),
            ...
            {
                var meth
 = methods.FirstOrDefault(m => MethodMatchesExact(m, argTypes, returnType));
                if (meth == null)
                    meth
 = methods.FirstOrDefault(m => MethodMatches(m, argTypes, returnType));
                return meth;
            }
 ...
 = opArgs;
            ...
            public override T Eval(EvaluatorState state)
            {
                try
                {
                    ...
                    {
                        Address addrInstr
 = opInst.EvalAddressBoxed(state);
                        object instance
 = addrInstr.GetBoxedValue(state);
                        object[] args
 = opArgs.Select(a => a.EvalBoxed(state)).ToArray();
                        result
 = method.Invoke(instance, args);
                    ...
                    {
                        object instance
 = opInst != null ? opInst.EvalBoxed(state) : null;
                        object[] args
 = opArgs.Select(a => a.EvalBoxed(state)).ToArray();
                        result
 = method.Invoke(instance, args);
 ...
 = opInst != null ? opInst.EvalBoxed(state) : null;
                    ...
                    addrs
 = opArgs.Select(a => a.EvalAddressBoxed(state)).ToArray();
                    args
 = addrs.Select(a => a.GetBoxedValue(state)).ToArray();
                    result
 = method.Invoke(instance, args);

                    for (int i
 = 0, n = args.Length; i < n; i++)
                    {
 ...
 = field;
            ...
            {
                var instance
 = this.operand != null ? this.operand.EvalBoxed(state) : null;
            ...
            {
                Address addr
 = this.operand != null ? this.operand.EvalAddressBoxed(state) : null;
 ...
 = field;
                ...
                {
                    object boxedInst
 = this.instance != null ? this.instance.GetBoxedValue(state) : null;
 ...
 = operand;
                this.fnGetter
 = (Func<T,V>)Delegate.CreateDelegate(typeof(Func<T, V>), property.GetGetMethod(true));
            }

            public override V Eval(EvaluatorState state)
            {
                T item
 = this.operand != null ? this.operand.Eval(state) : default(T);
 ...
 =
                ...
                fnCreateDelegate
 = (Func<Type, object, RuntimeMethodHandle, Delegate>)
        ...
        public class NewEvaluator<T> : Evaluator<T>
        {
            ...
            public NewEvaluator(ConstructorInfo constructor)
            {
                if (constructor != null)
                    this.fnConstructor
 = (Action<T>)CreateDelegate(typeof(Action<T>), constructor);
            }
 ...
 = (T)System.Runtime.Serialization.FormatterServices.GetUninitializedObject(typeof(T));
        ...
        public class NewEvaluator<T, A> : Evaluator<T>
        {
            ...
            {
                this.fnConstructor
 = (Action<T, A>)CreateDelegate(typeof(Action<T, A>), constructor);
                this.opArg
 = opArg;
            }
 ...
 = (T)System.Runtime.Serialization.FormatterServices.GetUninitializedObject(typeof(T));
        ...
        public class NewEvaluator<T, A1, A2> : Evaluator<T>
        {
            ...
            {
                this.fnConstructor
 = (Action<T, A1, A2>)CreateDelegate(typeof(Action<T, A1, A2>), constructor);
                this.opArg1
 = opArg1;
                this.opArg2
 = opArg2;
            ...
            {
                A1 a1
 = this.opArg1.Eval(state);
                A2 a2
 = this.opArg2.Eval(state);
                T t
 = (T)System.Runtime.Serialization.FormatterServices.GetUninitializedObject(typeof(T));
        ...
        public class NewEvaluator<T, A1, A2, A3> : Evaluator<T>
        {
            ...
            {
                this.fnConstructor
 = (Action<T, A1, A2, A3>)CreateDelegate(typeof(Action<T, A1, A2, A3>), constructor);
                this.opArg1
 = opArg1;
                this.opArg2
 = opArg2;
 ...
 = (T)System.Runtime.Serialization.FormatterServices.GetUninitializedObject(typeof(T));
        ...
        public class NewEvaluator<T, A1, A2, A3, A4> : Evaluator<T>
        {
            ...
            {
                this.fnConstructor
 = (Action)CreateDelegate(typeof(Action), constructor);
                this.opArg1
 = opArg1;
                this.opArg2
 = opArg2;
 ...
 = opArgs.Select(a => a.EvalBoxed(state)).ToArray();
        ...
        public class NewArrayInitEvaluator<T> : Evaluator<T[]>
        {
            ...
            {
                this.initializers
 = initializers.Select(i => (Evaluator<T>)i).ToArray();
            }

            public override T[] Eval(EvaluatorState state)
            {
                T[] array
 = new T[initializers.Length];
                for (int i
 = 0, n = initializers.Length; i < n; i++)
                {
                    array[i]
 = initializers[i].Eval(state);
                }
                return array;
            }
        }
        ...
        public class NewArrayBoundsEvaluator<T> : Evaluator<T[]>
        {
            ...
            {
                this.bounds
 = bounds.Select(b => (Evaluator<int>)b).ToArray();
            }

            public override T[] Eval(EvaluatorState state)
            {
                int[] bounds
 = this.bounds.Select(b => b.Eval(state)).ToArray();
 ...
 = evaluator;
            ...
            {
                T d
 = (T)(object)CreateDelegate(typeof(T), state, this.nArgs, this.evaluator);
                return d;
            }
        }
 ...
 = opArgs;
            ...
            {
                var function
 = opFunction.EvalBoxed(state);
                var args
 = opArgs.Select(a => a.EvalBoxed(state));
                try
                {
 ...
 = opNew;
                this.initializers
 = initializers.Select(i => (Initializer<T>)i).ToArray();
            }
 ...
 = evaluator;
            ...
            {
                var value
 = this.evaluator.EvalBoxed(state);
 ...
 = (T)boxedInstance;
        ...
        public class ValueTypePropertyAssignmentInitializer<T, V> : Initializer<T> where T : struct
        {
            ...
            {
                this.fnSetter
 = (Assigner)Delegate.CreateDelegate(typeof(Assigner), property.GetSetMethod(true));
                this.evaluator
 = evaluator;
            }
 ...
 = this.evaluator.Eval(state);
        ...
        public class PropertyAssignmentInitializer<T, V> : Initializer<T> where T : class
        {
            ...
            {
                this.fnSetter
 = (Action<T,V>)Delegate.CreateDelegate(typeof(Action<T,V>), property.GetSetMethod(true));
                this.evaluator
 = evaluator;
            }
 ...
 = field;
                this.initializers
 = initializers.Select(i => (Initializer<V>)i).ToArray();
            }
 ...
 = (V)field.GetValue(instance);
                for (int i
 = 0, n = this.initializers.Length; i < n; i++)
                {
 ...
 = instance;
                V value
 = (V)field.GetValue(boxedInstance);
                for (int i
 = 0, n = this.initializers.Length; i < n; i++)
                {
                    this.initializers[i].Init(state, ref value);
 ...
 = 0, n = this.initializers.Length; i < n; i++)
            ...
            where T : struct
        {
            ...
            {
                this.fnGetter
 = (Getter)Delegate.CreateDelegate(typeof(Getter), property.GetGetMethod(true));
                this.initializers
 = initializers.Select(i => (Initializer<V>)i).ToArray();
            }
 ...
 = evArgs;
            ...
            public override void Init(EvaluatorState state, ref T instance)
            {
                ...
                {
                    var args
 = evArgs.Select(a => a.EvalBoxed(state)).ToArray();
            ...
            where T : class
        {
            ...
            {
                this.fnAdder
 = (Action<T, A>)Delegate.CreateDelegate(typeof(Action<T, A>), addMethod);
                this.evArg
 = evArg;
            }
 ...
 = this.evArg.Eval(state);
            ...
            where T : class
        {
            ...
            {
                this.fnAdder
 = (Action<T, A1, A2>)Delegate.CreateDelegate(typeof(Action<T, A1, A2>), addMethod);
                this.evArg1
 = evArg1;
                this.evArg2
 = evArg2;
            ...
            {
                A1 arg1
 = this.evArg1.Eval(state);
                A2 arg2
 = this.evArg2.Eval(state);
        ...
        public static class Operators
        {
            ...
            {
                _methods
 = typeof(Operators).GetMethods(BindingFlags.Static | BindingFlags.Public).ToLookup(m => m.Name);
            }
