...
namespace IQToolkit
{
   public static class ExpressionExtensions
   {
      ...
      private static void ConvertExpressions(ref Expression expression1, ref Expression expression2)
      {
         ...
         {
            bool isNullable1 = TypeHelper.IsNullableType(expression1.Type);
            bool isNullable2 = TypeHelper.IsNullableType(expression2.Type);
            if (isNullable1 || isNullable2)
               if (TypeHelper.GetNonNullableType(expression1.Type) == TypeHelper.GetNonNullableType(expression2.Type))
               {
                  ...
                  else if (!isNullable2) expression2 = Expression.Convert(expression2, expression1.Type);
               }
         }
      }
      ...
      public static Expression Join(this IEnumerable<Expression> list, ExpressionType binarySeparator)
      {
         if (list != null)
         {
            Expression[] array = list.ToArray();
            if (array.Length > 0) return array.Aggregate((x1, x2) => Expression.MakeBinary(binarySeparator, x1, x2));
         }
