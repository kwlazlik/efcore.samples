...
namespace IQToolkit
{
   ...
   public class ExpressionWriter : ExpressionVisitor
   {
      ...
      protected ExpressionWriter(TextWriter writer) => this.writer = writer;

      protected int IndentationWidth { get; set; } = 2;

      public static void Write(TextWriter writer, Expression expression) => new ExpressionWriter(writer).Visit(expression);
      ...
      protected void Write(string text)
      {
         if (text.IndexOf('\n') >= 0)
         {
            ...
            {
               Write(lines[i]);
               if (i < n - 1) WriteLine(Indentation.Same);
            }
         }
         else
         {
      ...
      protected virtual string GetTypeName(Type type)
      {
         ...
         if (iGeneneric > 0) name = name.Substring(0, iGeneneric);
         if (type.IsGenericType || type.IsGenericTypeDefinition)
         {
            ...
            {
               if (i > 0) sb.Append(",");
               if (type.IsGenericType) sb.Append(GetTypeName(args[i]));
            }
      ...
      protected override Expression VisitConstant(ConstantExpression c)
      {
         ...
         else if (c.Type.IsArray)
         {
            ...
            VisitNewArray(Expression.NewArrayInit(elementType,
