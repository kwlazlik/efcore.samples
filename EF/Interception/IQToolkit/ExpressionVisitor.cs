...
namespace IQToolkit
{
   public abstract class ExpressionVisitor
   {
      ...
      {
         if (u.Operand != operand || u.Type != resultType || u.Method != method) return Expression.MakeUnary(u.NodeType, operand, resultType, method);
         return u;
      }
      ...
      protected BinaryExpression UpdateBinary(BinaryExpression b, Expression left, Expression right, Expression conversion, bool isLiftedToNull,
                                              MethodInfo method)
      {
      ...
      {
         if (expression != b.Expression || typeOperand != b.TypeOperand) return Expression.TypeIs(expression, typeOperand);
         return b;
      }
      ...
      {
         if (expression != m.Expression || member != m.Member) return Expression.MakeMemberAccess(expression, member);
         return m;
      }
      ...
      {
         if (obj != m.Object || method != m.Method || args != m.Arguments) return Expression.Call(obj, method, args);
         return m;
      }
         ...
         if (original != null)
         {
            ...
            for (int i = 0, n = original.Count; i < n; i++)
            {
               ...
               else if (p != original[i])
               {
                  ...
                  for (int j = 0; j < i; j++) list.Add(original[j]);
                  list.Add(p);
               }
            }
      ...
      protected virtual ReadOnlyCollection<Expression> VisitMemberAndExpressionList(ReadOnlyCollection<MemberInfo> members,
      ...
      {
         if (expression != assignment.Expression || member != assignment.Member) return Expression.Bind(member, expression);
         return assignment;
      }
      ...
      {
         if (bindings != binding.Bindings || member != binding.Member) return Expression.MemberBind(member, bindings);
         return binding;
      }
      ...
      {
         if (initializers != binding.Initializers || member != binding.Member) return Expression.ListBind(member, initializers);
         return binding;
      }
      ...
      {
         if (body != lambda.Body || parameters != lambda.Parameters || delegateType != lambda.Type) return Expression.Lambda(delegateType, body, parameters);
         return lambda;
      }
      ...
      {
         if (nex != init.NewExpression || bindings != init.Bindings) return Expression.MemberInit(nex, bindings);
         return init;
      }
      ...
      {
         if (nex != init.NewExpression || initializers != init.Initializers) return Expression.ListInit(nex, initializers);
         return init;
      }
      ...
      {
         if (args != iv.Arguments || expression != iv.Expression) return Expression.Invoke(expression, args);
         return iv;
      }
   }
}
