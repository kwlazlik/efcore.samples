...
namespace IQToolkit
{
   ...
   {
      public static IEnumerable<T> Sort<T>(this IEnumerable<T> items, Func<T, IEnumerable<T>> fnItemsBeforeMe) => Sort(items, fnItemsBeforeMe, null);
      ...
      public static IEnumerable<T> Sort<T>(this IEnumerable<T> items, Func<T, IEnumerable<T>> fnItemsBeforeMe, IEqualityComparer<T> comparer)
      {
         ...
         foreach (T item in items) SortItem(item, fnItemsBeforeMe, seen, done, result);
         return result;
      }
      ...
      private static void SortItem<T>(T item, Func<T, IEnumerable<T>> fnItemsBeforeMe, HashSet<T> seen, HashSet<T> done, List<T> result)
      {
         ...
         {
            if (seen.Contains(item)) throw new InvalidOperationException("Cycle in topological sort");
            seen.Add(item);
